...
public class TimeComplexity {
    ...
    public static <T> void computeAndIsSorted(Class<T> classType, int[] data, int n) {
        computeAndIsSorted(classType, data, n, null);
    }
    ...
    public static <T> void computeAndIsSorted(Class<T> classType, int[] data, int n, String message) {
        ...
        if (MergeSort.class.getSimpleName().equals(classSimpleName)) {
            MergeSort.sort(data);
        } else if (QuickSort.class.getSimpleName().equals(classSimpleName)) {
            QuickSort.sort(data);
        } else if (BubbleSort.class.getSimpleName().equals(classSimpleName)) {
            BubbleSort.sort(data);
        } else if (InsertSort.class.getSimpleName().equals(classSimpleName)) {
            InsertSort.sort(data);
        } else if (QuickSortThreeWay.class.getSimpleName().equals(classSimpleName)) {
            QuickSortThreeWay.sort(data);
        }
    ...
    private static void printInfo(String algorithmName, int[] data, int n, long start, long end, String message) {
        ...
        System.out.printf("%s, n=%s%s, spendTime:%fs(%dns) ", algorithmName, str.reverse(), message == null ? "" : "(" + message + ")", second, nanoSecond);
